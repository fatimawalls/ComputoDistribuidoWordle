...
// ---------- PALABRA NO REPETIDA ----------
char* generar_palabra_no_repetida(const char *usuario) {

    static char palabra[20];

    char words[100][20];
    int count = 0;

    obtener_palabras_usuario(usuario, words, &count);

    int intentos = 0;

    while (intentos < 100) {
        const char *p = palabra_random();

        if (!palabra_ya_resuelta(words, count, p)) {
            strcpy(palabra, p);
            return palabra;
        }
...
void jugar_partida(int client_sock, char *usuario) {
    ...
    char *secreta = generar_palabra_no_repetida(usuario);

    printf("🎯 Palabra para %s: %s\n", usuario, secreta);
    ...
    while (intentos < MAX_ATTEMPTS) {
        ...
        if (intentos == MAX_ATTEMPTS - 1) {
            ...
            if (strcmp(word, secreta) == 0) {
                ...
                agregar_palabra_usuario(usuario, secreta);
            } else {
                snprintf(msg, sizeof(msg),
                    "{\"type\":\"result\",\"data\":\"%s\",\"status\":\"lose\"}\n",
                    resultado);
...
int main() {
    ...
    printf("🔥 Servidor JSON PRO corriendo en puerto %d...\n", PORT);
